...
module.exports = function (router) {
    ...
    router.post("/messages", function (req, res) {
        onlyForLoggedInUsers(req, res, function (sessionData) {
            csrfProtected(req, res, sessionData, function () {
    ...
    function csrfProtected(req, res, sessionData, fn) {
        var isCsrfTokenValid = true;
        var token = req.param("csrfToken");
        /**
         ADD CSRF SUPPORT HERE
         user the tokenizer `csrfTokenizer.validate(token, sessionData.user.id)` method to validate the token
         **/
        if (isCsrfTokenValid) {
            fn();
        } else {
